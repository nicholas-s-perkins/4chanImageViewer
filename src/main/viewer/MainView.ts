module Viewer{
    ...
    export class MainView{
        ...
        private mainView:DomUtil = null;
        private mainImg:DomUtil = null;
        private centerBox:DomUtil = null;
        private topLayer:DomUtil = null;
        private customStyle:DomUtil = null;
        private textWrapper:DomUtil = null;
        private leftArrow:DomUtil = null;
        private rightArrow:DomUtil = null;
        private bottomMenu:DomUtil = null;
        ...
        private mouseTimer:number = null;
        ...
        constructor(imagePostIndex?:number) {
            ...
            var currentThreadId = DomUtil.get('.thread').id;
            ...
            //add handlers
            this.centerBox.on('click',()=>{this.confirmExit()});
            this.textWrapper.on('click',(event:UIEvent)=>{ this.eventStopper(event);});
            this.bottomMenu.on('click',()=>{ this.menuClickHandler();});
            ...
            UNSAFE_WINDOW
                .on('keydown',(event:KeyboardEvent)=>{
                    this.arrowKeyListener(event);
                })
                .on('mousemove',(event:MouseEvent)=>{
                    this.menuWatcher(event)
                });
        }
        ...
        menuInit(){
            ...
            menuControls.each((input:HTMLInputElement)=>{
        ...
        runImagePreloading(index:number){
            if(this && index < this.postData.length){
                if(this.canPreload){
                    ...
                    var loadFunc = ()=>{this.runImagePreloading(index+1);};
                    ...
                    if( this.postData[index].tagType === TagType.VIDEO){
                        ...
                        var newImage:any = document.createElement(this.postData[index].tagTypeName);
        ...
        replaceElement(element:DomUtil,newTagType:string){
            var rawElement = <HTMLVideoElement>element.elementList[0];

            var newElement = DomUtil.createElement(newTagType,{
                id:element.id,
        ...
        fitHeightToScreen(){
            ...
            var height:number = this.mainImg.getAttr('naturalHeight');
